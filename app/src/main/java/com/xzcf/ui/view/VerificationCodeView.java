...
public class VerificationCodeView extends View {
    ...
    private float offset = 2;//扭曲偏移
    private String vCode;
    private boolean isNetCode = false;//是否是网络请求到验证码
    private int mInterferenceLineCount = 0;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            isWrapContent = true;
            mWidth = (int) (textPaint.measureText("0") * 1.5 * codeNum + 20 + (int) UiUtils.dp2px(getContext(), 8));
        }
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            mHeight = (int) UiUtils.dp2px(getContext(), 40);
        }
    ...
    private void initView() {
        ...
        maxTextSize = UiUtils.sp2px(getContext(), 40);
        defaultTextSize = UiUtils.sp2px(getContext(), 20);
        textPaint = new Paint();
    ...
    private void createCodeBitmap() {
        ...
        if (!isWrapContent) {
            ...
            horizontalOffset = (int) ((mWidth - drawWidth - (int) UiUtils.dp2px(getContext(), 8)) / 2);
            if (horizontalOffset < 0) {
                horizontalOffset = 0;
            }
        }
        // 生成干扰线坐标
        for (int i = 0; i < mInterferenceLineCount; i++) {
            Path path = new Path();
    ...
    private void dynamicSetTextPaint(String text) {
        int avaiWidth = (int) (mWidth * 2.0f / 3 - getPaddingLeft() - getPaddingRight() - (int) UiUtils.dp2px(getContext(), 2));
        if (avaiWidth <= 0) {
            return;
        }
