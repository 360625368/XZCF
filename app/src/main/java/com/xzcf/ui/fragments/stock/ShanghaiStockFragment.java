...
public class ShanghaiStockFragment extends BaseSuperFragment {
    ...
    private final String stockType= "1";

    public static ShanghaiStockFragment newInstance() {
        Bundle args = new Bundle();
        ShanghaiStockFragment fragment = new ShanghaiStockFragment();
...
//            initRankData();
    ...
    private void intervalUpdateData(){
        try{
            mIntervalDisposable = Flowable.interval(INTERVAL_TIME, TimeUnit.SECONDS)
                    .subscribe(aLong -> {
                        // 涨跌幅
                        initRankData();
                    });
