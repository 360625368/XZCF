...
public class FundsFlowActivity extends BaseActivity {
    ...
    public static Intent startAction(Context context) {
        return new Intent(context, FundsFlowActivity.class);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mColumnHeaders.add("币种" );
        mColumnHeaders.add("证券名称" );
        mColumnHeaders.add("成交日期" );
        mColumnHeaders.add("成交价格" );
        mColumnHeaders.add("成交数量" );
        mColumnHeaders.add("发生金额" );
        mColumnHeaders.add("合同编号" );
        mColumnHeaders.add("业务名称" );
        mColumnHeaders.add("手续费" );
        mColumnHeaders.add("印花税" );
        mColumnHeaders.add("过户费" );
    ...
    private List<List<String>> bargainRecord2Cells(List<BargainRecordResponse.LogInfosBean> logInfos) {
        ...
        for (BargainRecordResponse.LogInfosBean logInfo : logInfos) {
            ...
            cells.add(String.format(Locale.getDefault(), "证券%s(%s)", logInfo.getType(), logInfo.getStockName()));
            cells.add(logInfo.getCommission());
            cells.add(logInfo.getStampDuty());
            cells.add(logInfo.getTransferFee());
